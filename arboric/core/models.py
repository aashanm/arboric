...
class Workload(BaseModel):
    ...
    power_draw_kw: float = Field(..., gt=0, le=10000, description="Average power consumption in kilowatts")
    deadline_hours: float = Field(..., gt=0, le=720, description="Must complete within this many hours")
    workload_type: WorkloadType = Field(default=WorkloadType.GENERIC, description="Workload classification")
    priority: WorkloadPriority = Field(default=WorkloadPriority.NORMAL, description="Scheduling priority")
...
class GridWindow(BaseModel):
    ...
    renewable_percentage: float = Field(default=0, ge=0, le=100, description="Percentage of renewable sources")
...
class ScheduleResult(BaseModel):
    ...
    optimized_avg_carbon: float = Field(..., ge=0, description="Average carbon during optimal window")
    ...
    baseline_avg_carbon: float = Field(..., ge=0, description="Average carbon during baseline window")
